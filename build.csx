...
Console.WriteLine();
if (RunCommand("dotnet", $"restore ./{NugetIntegrationTestsProject} --packages ./packages --configfile \"nuget.integration-tests.config\"") != 0)
{
  Environment.Exit(1);
}
...
Console.WriteLine();
if (RunCommand("dotnet", $"build ./{NugetIntegrationTestsProject} -c Release --packages ./packages --no-restore") != 0)
{
  Environment.Exit(1);
}
...
Console.WriteLine();
if (RunCommand("dotnet", $"test ./{NugetIntegrationTestsProject} -c Release --no-build --no-restore") != 0)
{
  Environment.Exit(1);
}
...
void RewriteNugetIntegrationsTestProject()
{
  ...
  var lines = File.ReadAllLines($"./{IntegrationTestsProject}/{IntegrationTestsProject}.csproj");
  ...
  Directory.CreateDirectory($"./{NugetIntegrationTestsProject}");
  File.WriteAllLines($"./{NugetIntegrationTestsProject}/{NugetIntegrationTestsProject}.csproj", lines);
}
